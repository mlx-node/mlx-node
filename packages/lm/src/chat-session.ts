 ...
 * decoded text, tool-call list, and reasoning body. The assistant
 * entry is appended to `this.history` after every successful turn
 * and is later read back by the native `chatSessionStart` cold-replay
 * path (image-change mid-session restart, `startFromHistory*`,
 * server-side `SessionRegistry` cache-miss rebuild). Dropping either
 * `toolCalls` or `reasoningContent` changes the rendered assistant
 * bytes on replay: tool responses lose their declaring call, and
 * thinking-capable templates lose the thought channel that preceded
 * the call.
 */
...
export class ChatSession<M extends SessionCapableModel = SessionCapableModel> {
  ...
  async send(userMessage: string, opts: SendOptions = {}): Promise<ChatResult> {
    ...
    try {
      ...
      this.history.push(buildAssistantMessage(result.text, result.toolCalls, result.thinking));
      this.turnCount++;
    ...
    try {
      ...
      try {
        ...
        if (sawFinal && !delegated) {
          this.history.push(pendingUser);
          this.history.push(buildAssistantMessage(finalRaw || accumulatedVisible, finalToolCalls, finalThinking));
          this.turnCount++;
