...
describe('delegate runtime permission handoff', () => {
  it.each(['missing-caller', 'sandbox-denied', 'pipeline-denied', 'authorized'] as const)(
    'handles %s in the real agent loop without permission retry turns',
    async (scenario) => {
      ...
      if (scenario !== 'missing-caller') {
        vi.stubEnv('CODEX_THREAD_ID', 'test-thread');
      ...
      try {
        ...
        expect(inferences).toBe(scenario === 'authorized' ? 2 : 1);
        ...
        expect(handoffs).toHaveLength(scenario === 'authorized' ? 0 : 1);
        if (scenario !== 'authorized') expect(process.exitCode).toBe(1);
      } finally {
        session?.dispose();
