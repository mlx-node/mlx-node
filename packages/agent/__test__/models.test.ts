...
beforeAll(async () => {
  ...
  await writeModelDir('lfm-moe', { model_type: 'lfm2_moe' }); // no LAUNCH_PRESETS entry
...
describe('discoverMlxModels', () => {
  it('returns only chat-capable model dirs, sorted by name', () => {
    expect(infos.map((m) => m.discovered.name)).toEqual(['alpha-qwen35', 'beta-gemma', 'gamma-fallback']);
  });
  ...
  it('flags reasoning per family: qwen3_5 yes, gemma4 no', () => {
