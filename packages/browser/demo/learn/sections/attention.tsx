...
export function AttentionMemoryWallSection() {
  return (
    <Prose>
      ...
      <p className="text-muted-foreground">
        One caveat for honesty: this only bites during <em>prefill</em> — digesting a long prompt in one pass — and only
        on the 6 full-attention layers. Single-token decode (one new query row, the part you watch in the chapter demo)
        and the 18 GatedDeltaNet layers never build an N×N matrix at all.
      </p>
      <p>
  ...
  return (
    <Prose>
      ...
      <p>
        ...
        operation, not an approximation like sparse or low-rank attention — without ever materializing the N×N matrix. It
        tiles <code>Q</code>, <code>K</code>, and <code>V</code> into small blocks and, for each query tile, streams over
        the key tiles while keeping a small running state per query row: two single numbers — the max score so far{' '}
      ...
      <p className="text-muted-foreground">
        ...
        your in-browser Qwen runs during prefill is a{' '}
        <strong>FlashAttention-2-style</strong> tile kernel — v3&apos;s tricks are specific to NVIDIA&apos;s Hopper GPUs
        and FP8, so they don&apos;t apply here.
      </p>
      ...
      <p>
        ...
        numbers. Softmax turns a row of scores into weights that sum to 1 by exponentiating each score and dividing by
        the total. Take the row <code>[2, 1, 3]</code>: the exponentials are <MathDisplay inline latex={String.raw`e^{2} \approx 7.39`} />,{' '}
        <MathDisplay inline latex={String.raw`e^{1} \approx 2.72`} />, <MathDisplay inline latex={String.raw`e^{3} \approx 20.09`} />; their sum is <code>30.19</code>; divide through and the
        weights are <code>0.245, 0.090, 0.665</code>. The biggest score wins most of the weight — that is the whole
        job.
      </p>
      <p>
        One practical wrinkle: <MathDisplay inline latex={String.raw`e^{x}`} /> explodes. <MathDisplay inline latex={String.raw`e^{89}`} /> already overflows a 32-bit float (the
        limit is about <MathDisplay inline latex={String.raw`e^{88.7}`} />), and raw attention scores can get big. The standard fix is to subtract the
        row&apos;s max from every score before exponentiating — the largest term becomes <MathDisplay inline latex={String.raw`e^{0} = 1`} /> and
        nothing overflows. Dividing by the sum cancels the shift, so the weights come out identical. But notice what
        that fix costs: you need the <em>max of the whole row</em> before you can exponentiate <em>anything</em>.
        Hence two passes — one to find the max, one to sum the exponentials.
      </p>
      <p>
        ...
        multiplying it by <MathDisplay inline latex={String.raw`e^{\,\text{old max}\; -\; \text{new max}}`} />. Why is that the exact right patch? Every term you
        accumulated has the form <MathDisplay inline latex={String.raw`e^{\,\text{score}\; -\; \text{old max}}`} />, and
      </p>
      <MathDisplay latex={String.raw`e^{\,s - m_{\text{old}}} \cdot e^{\,m_{\text{old}} - m_{\text{new}}} = e^{\,s - m_{\text{new}}}`} />
      <p>
        — one multiply re-expresses every old term against the new max, as if you had known it all along. Concretely,
        in the animation below the first chunk <code>[2, 1, 3]</code> finishes with max <code>3</code>; then chunk two
        contains a <code>5</code>, so the running sum and the running output are both multiplied by{' '}
        <MathDisplay inline latex={String.raw`e^{\,3 - 5} \approx 0.135`} /> before the new terms fold in. The 2021 &ldquo;memory-efficient attention&rdquo;
        paper noticed you can run this over <em>chunks</em> of keys and values — so you never need the whole row in
        memory at once.
      </p>
      ...
      <p>
        ...
        <code>o</code> is itself a <code>128 × 64</code> tile, another 32 KB (only <code>m</code> and <code>ℓ</code>{' '}
        are single numbers per row). Add it up: ≈ 190 KB, right at the edge of the ~192 KB of SRAM sitting next to the
        compute units — which is exactly why the tiles are this size and not bigger. Production kernels squeeze further
        with 16-bit tiles and narrower key blocks, but the budget arithmetic — &ldquo;what fits in SRAM&rdquo; — is
        what picks the tile sizes.
      </p>
      <p className="text-muted-foreground">
      ...
      <p>
        ...
        weight <code>V</code>. For this model&apos;s 8 heads in bf16 that is ≈ 1.07 GB of bus traffic <em>per
        full-attention layer</em>, for scores that are used once and thrown away. The tiled kernel moves exactly zero
        of those bytes. Drag the sequence length and watch the gap:
      </p>
      ...
      <p>
        The next two versions are about <em>filling the machine</em>, so you need a picture of the machine. A GPU is
        not one processor — it is more like an office park. An NVIDIA A100 has <strong>108 SMs</strong> (streaming
        ...
        which execute in lockstep. Two consequences matter here: an SM with no block assigned does <em>nothing</em>,
        and the kernel decides how many blocks exist. Zoom through the levels:
      </p>
      ...
      <p>
        ...
        × attention head): one prompt times 8 heads is <strong>8 blocks</strong> — on a 108-SM A100, about 7% of the
        chip working and 100 SMs sitting dark. v2&apos;s headline fix is to <em>also</em> split the query sequence into
      ...
      <p>
        ...
        <code>ℓ</code>-division that is deferred), and for causal masks (where future tokens cannot be attended
        anyway) whole score tiles that would be fully masked are simply never computed — measured at ~1.7–1.8× by
        itself. Together: about twice as fast as v1, with the attention kernel hitting up to 73% of the A100&apos;s
        theoretical FP16 peak. (That occupancy idea is exactly what drives the decode story below.)
      </p>
      ...
      <p>
        FlashAttention-3 (2024) is tuned for NVIDIA&apos;s Hopper GPUs (the H100), and it starts from a wild
        imbalance in the hardware. The H100&apos;s Tensor Cores deliver about <strong>989 TFLOPS</strong> of fp16
        matrix multiply — but the little units that compute exponentials (which every softmax needs) manage only about{' '}
      ...
      <p>
        ...
        sounds innocent until you see what it costs. An fp8 number (the E4M3 format) has 256 possible bit patterns and
        a maximum value of 448; matmuls in fp8 run about twice as fast as fp16. The catch: with so few representable
        ...
        levels and their information is gone. FlashAttention-3 rescues fp8 twice: <em>block quantization</em> gives
        each small tile its own scale, so one outlier only coarsens its own block — and <em>incoherent processing</em>{' '}
      ...
      <p>
        ...
        NVIDIA&apos;s next generation, the Blackwell B200 — where matmul throughput roughly doubled again while the
        rest of the chip didn&apos;t — reaching up to 1,613 TFLOPS (71% utilization). Our deep-dive stops at v3 because
        v4&apos;s tricks are Blackwell-specific, but notice the pattern across every version: the <em>algorithm&apos;s
        invariant never changes</em> — the N×N matrix never touches slow memory — and each release just re-tunes that
        invariant to whatever the newest silicon&apos;s bottleneck happens to be.
      </p>
      ...
      <p>
        ...
        <em>not</em> use it: at 0.8B with 8 query heads the fused kernel would launch only 8 GPU workgroups and leave most
        of the chip idle, so an occupancy gate routes decode back to the plain three-step path (matmul → softmax →
...
export function AttentionRooflineSection() {
  return (
    <Prose>
      ...
      <p>
        The <a href="/chapters/attention/memory-wall">memory wall</a> kept leaning on one word —{' '}
        <em>memory-bound</em> — to explain why attention, and single-token decode in general, spends its time waiting on
        memory instead of computing. That was a story. The <strong>roofline</strong> turns it into a number you can plot,
        and that number tells you exactly when the story stops being true.
      </p>
      ...
      <p>
        ...
        byte and you finish the math long before the next bytes arrive — you are <strong>memory-bound</strong>, idling on
        the left of the ridge. Do <em>more</em> and the bytes arrive faster than you can crunch them — you are{' '}
      ...
      <p>
        The two halves of inference sit on opposite sides. <strong>Prefill</strong> digests the whole prompt at once, so
        every weight it reads is reused across <em>hundreds</em> of token positions in parallel — a mountain of matmul
        per byte. It sits comfortably right of the ridge, <strong>compute-bound</strong>. <strong>Decode</strong> at
        batch 1 is the opposite extreme: to produce <em>one</em> token it streams the entire model&apos;s weights through
        the chip exactly once, doing almost no math per byte. It falls off the far-left cliff,{' '}
        <strong>deeply memory-bound</strong> — the same idle-waiting-on-memory the rest of this course keeps running into.
      </p>
      <p>
        Drag the batch slider and watch the decode dot march. Batching <code>N</code> requests lets all{' '}
        <code>N</code> reuse each weight read, so intensity climbs roughly <code>×N</code> and the dot slides right up
        the diagonal toward the ridge — flipping from memory-bound to compute-bound only once there&apos;s finally enough
        math per byte to keep the FLOPs busy:
      </p>
