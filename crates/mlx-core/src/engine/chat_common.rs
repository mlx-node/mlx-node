            ...
            let next_y = if step + 1 < $max {
                ...
                let (next_token, budget_forced) =
                    if $tracker.should_force_think_end() {
                        ...
                        logits = $crate::engine::chat_common::apply_all_penalties(
                            logits, &$hist, &$p,
                        )?;
                        $profiler.end();
                ...
                if !budget_forced
                    && $crate::engine::chat_common::mtp_trace_logits()
                {
                    ...
                    match $crate::engine::chat_common::trace_top2(
                        &logits_1d, vocab,
                    ) {
                        Ok(t2) => {
                            next_token.eval();
...
macro_rules! decode_loop_mtp {
    (
        ...
        let chained_cycles_enabled: bool =
            $crate::engine::chat_common::mtp_chained_cycles_enabled();
        ...
        loop {
            ...
            let commit_anchor = if cycle_seed_was_chained && $mtp.committed_history_active {
                $crate::engine::chat_common::MtpCommitAnchor::SkipAlreadyCommittedAnchor
            } else {
                $crate::engine::chat_common::MtpCommitAnchor::IncludeAnchor
            };
            ...
            let cycle_res =
                $crate::engine::chat_common::run_mtp_cycle_inner(
                    &mut $mtp,
                    prev_h,
                    prev_e,
                    last_id,
