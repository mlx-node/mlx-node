...
impl PagedKVCacheAdapter {
    ...
    /// Transfer flat per-layer KV slabs into the paged Metal pool
    /// (Phase 4 piece 2 helper).
    ///
    /// The Rust flat decode path stores per-layer K/V tensors of shape
    /// `[1, num_kv_heads, num_tokens, head_dim]`. To hand off into the C++
    /// paged decode FFI we must materialize those tokens in the per-layer
    /// pool (`LayerKVPool`) at the slots of the pre-recorded paged blocks.
    /// This helper performs that bulk transfer:
    ///
    /// 1. For each layer, slice the flat slab `[B=1, H, T, D]` into
    ///    `block_size`-sized chunks along the token axis. The final chunk
    ///    may be partial (length `T % block_size`).
    /// 2. Transpose each chunk to the kernel-expected
    ///    `[chunk_size, num_kv_heads, head_size]` layout.
    /// 3. Build a `slot_mapping` `[block_id * block_size + j for j in
    ///    0..chunk_size]` referencing the block already recorded in
    ///    `block_table`.
    /// 4. Dispatch `LayerKVPool::write_kv` (the same `reshape_and_cache`
    ///    kernel that decode uses), which lays out the chunk in vLLM's
    ///    on-GPU paged format.
    ///
    /// Cross-request prefix-cache registration:
    /// - `request_tokens` and `block_table.num_tokens` are seeded with
    ///   the prefill token sequence so a follow-up
    ///   `register_full_blocks_for_reuse` (or `finalize_turn_keep_live`)
    ///   call can publish the FULL blocks for cross-conversation reuse.
    /// - This mirrors the post-`record_tokens` invariant the rest of the
    ///   adapter API expects; callers don't need to call `record_tokens`
    ///   themselves.
    ///
    /// # Caller contract
    ///
    /// - Adapter MUST be in a freshly-reset state — `reset_for_new_request`
    ///   was just called and no `find_cached_prefix` /
    ///   `allocate_suffix_blocks` / `record_tokens` have run yet. The
    ///   helper allocates blocks itself and writes through every layer
    ///   from the flat slabs on its own. It also rejects a non-empty
    ///   `request_tokens` rather than silently overwriting.
    /// - `flat_layer_kv.len()` must equal `LayerKVPool::num_layers()`. A
    ///   layer with no flat KV (e.g. linear-attention GDN layers) MUST be
    ///   passed as `None`; the helper skips it.
    /// - `request_tokens.len() == num_tokens`. The token slice is used
    ///   for prefix-hash registration of full blocks.
    ///
    /// # Errors
    ///
    /// - `request_tokens.len() != num_tokens`.
    /// - `flat_layer_kv.len()` mismatched with the pool's layer count.
    /// - `BlockAllocator` exhausted before all blocks could be allocated
    ///   (caller-visible `block_table` is left intact for cleanup via
    ///   `release_request`).
    /// - Underlying `write_kv` failure (Metal unavailable, dtype mismatch,
    ///   etc.).
    ///
    /// # Performance
    ///
    /// `num_tokens / block_size * num_full_attn_layers` synchronous kernel
    /// dispatches. For a 4K-token prompt × 10 full-attention layers (Qwen
    /// 3.5 MoE has 10 of 40 layers as full-attention) that's 256 × 10 =
    /// 2,560 dispatches — sub-second on M3 Max in practice.
    ///
    /// # Layer routing (decoupled from model types)
    ///
    /// The adapter doesn't know anything about model layer kinds; the
    /// caller decides which slots get a `Some((K, V))` (full-attention)
    /// vs. `None` (linear-attention / no paged KV). For Qwen3.5 MoE the
    /// caller passes `None` for every `is_linear_layer(i)` slot.
    #[cfg(target_os = "macos")]
    pub fn transfer_flat_to_paged(
        &mut self,
        flat_layer_kv: &[Option<(&MxArray, &MxArray)>],
        num_tokens: u32,
        request_tokens: &[u32],
    ) -> Result<(), String> {
        ...
        if self.block_table.is_none() {
            return Err(
                "transfer_flat_to_paged called before reset_for_new_request \
                 (no active request)"
                    .to_string(),
            );
        }
        if !self.request_tokens.is_empty() {
            return Err(format!(
                "transfer_flat_to_paged: adapter is not in a freshly-reset state \
                 (request_tokens.len() = {}). Call reset_for_new_request first \
                 to start a new request.",
                self.request_tokens.len(),
            ));
        }
        if request_tokens.len() != num_tokens as usize {
            return Err(format!(
                "transfer_flat_to_paged: request_tokens.len() = {} but num_tokens \
                 = {}; the slice must contain exactly the prefill token sequence",
                request_tokens.len(),
                num_tokens,
            ));
        }
        let pool_layer_count = self.layer_kv_pool.num_layers();
        if flat_layer_kv.len() != pool_layer_count {
            return Err(format!(
                "transfer_flat_to_paged: flat_layer_kv.len() = {} but pool has \
                 {} layers; pass None for layers with no paged KV",
                flat_layer_kv.len(),
                pool_layer_count,
            ));
        }
        if num_tokens == 0 {
            // Nothing to transfer; leave the adapter in its freshly-reset
            // state so the caller can still call register_full_blocks etc.
            return Ok(());
        }
        ...
        self.allocate_suffix_blocks(num_tokens)?;

        // Seed the request_tokens / block_table.num_tokens. Mirrors the
        // post-`find_cached_prefix` + `record_tokens` invariants so a
        // follow-up `register_full_blocks_for_reuse` works.
        self.request_tokens.extend_from_slice(request_tokens);
        if let Some(table) = self.block_table.as_mut() {
        ...
        for (layer_idx, slot) in flat_layer_kv.iter().enumerate() {
            ...
            // paged pool. C++ `init_paged` stores zero placeholders for
            // these slots and ignores them in the compiled graph.
            let Some((keys_full, values_full)) = slot else {
                continue;
            };

            // Trim the flat slab to the prefilled prefix `[..num_tokens]`
            // along the token axis. KVCache pre-allocates 256-token chunks,
            // so the underlying tensor is usually larger than `num_tokens`.
            let keys = keys_full
                .slice_axis(2, 0, num_tokens as i64)
                .map_err(|e| format!("transfer_flat_to_paged layer {layer_idx} keys slice: {e}"))?;
            let values = values_full
                .slice_axis(2, 0, num_tokens as i64)
                .map_err(|e| {
                    format!("transfer_flat_to_paged layer {layer_idx} values slice: {e}")
                })?;

            // Squeeze batch (axis 0) so we can transpose to
            // [num_tokens, num_kv_heads, head_dim] (the kernel's
            // contiguous input layout).
            //
            // Source layout after slice:  [B=1, num_kv_heads, num_tokens, head_dim]
            // Squeeze + transpose:        [num_tokens, num_kv_heads, head_dim]
            let keys_3d = keys
                .squeeze(Some(&[0]))
                .map_err(|e| format!("transfer_flat_to_paged layer {layer_idx} keys squeeze: {e}"))?
                .transpose(Some(&[1, 0, 2]))
                .map_err(|e| {
                    format!("transfer_flat_to_paged layer {layer_idx} keys transpose: {e}")
                })?;
            let values_3d = values
                .squeeze(Some(&[0]))
                .map_err(|e| {
                    format!("transfer_flat_to_paged layer {layer_idx} values squeeze: {e}")
                })?
                .transpose(Some(&[1, 0, 2]))
                .map_err(|e| {
                    format!("transfer_flat_to_paged layer {layer_idx} values transpose: {e}")
                })?;

            // Force evaluation so the kernel dispatch sees materialized
            // input buffers. `write_kv` reads the buffers directly via
            // `MlxMetalBuffer::from_mlx_array` which fails if the array
            // is still lazy.
            keys_3d.eval();
            values_3d.eval();

            // Walk block_size-sized chunks along the token axis.
            ...
            while written < num_tokens {
                ...
                let chunk_keys = keys_3d
                    ...
                    .map_err(|e| {
                        format!(
                            "transfer_flat_to_paged layer {layer_idx} chunk keys slice \
                ...
                let chunk_values = values_3d
                    ...
                    .map_err(|e| {
                        format!(
                            "transfer_flat_to_paged layer {layer_idx} chunk values slice \
                             at written={written} chunk_size={chunk_size}: {e}"
                        )
                    })?;
                chunk_keys.eval();
                ...
                // Build slot_mapping for this chunk. The block hosting
                // logical position `written` is at table_idx
                // `written / block_size`; its `block_id` is the physical
                // block id.
                let slot_mapping = self.build_slot_mapping(written, chunk_size).map_err(|e| {
                    format!(
                        "transfer_flat_to_paged layer {layer_idx} build_slot_mapping \
                         at written={written}: {e}"
                    )
                })?;
                ...
                unsafe {
                    self.layer_kv_pool.write_kv(
                        layer_idx as u32,
                        chunk_keys.as_raw_ptr(),
                        chunk_values.as_raw_ptr(),
                        &slot_mapping,
                        input_metal_dtype,
                        /* k_scale */ 1.0,
                        /* v_scale */ 1.0,
    ...
    /// Non-macOS stub: the underlying `write_kv` kernel is macOS-only.
    #[cfg(not(target_os = "macos"))]
    pub fn transfer_flat_to_paged(
        &mut self,
        _flat_layer_kv: &[Option<(&MxArray, &MxArray)>],
        _num_tokens: u32,
        _request_tokens: &[u32],
    ) -> Result<(), String> {
        Err("transfer_flat_to_paged is only supported on macOS (Metal backend)".to_string())
    }
}
...
mod tests {
    ...
    //
    // Phase 4 piece 2 helper that bulk-writes flat per-layer KVCache
    // slabs into the paged Metal pool. Validation tests use the
    // freshly-reset adapter and probe the contract checks; the
    // happy-path Metal dispatch test gracefully skips on no-Metal
    // hosts.

    /// Reject calls before `reset_for_new_request`.
    #[test]
    fn test_transfer_flat_to_paged_rejects_no_active_request() {
        let allocator = new_allocator(8, 16);
        let Some(mut adapter) = maybe_adapter(allocator, 16) else {
            eprintln!(
                "skipping test_transfer_flat_to_paged_rejects_no_active_request: \
                 Metal device unavailable"
            );
            return;
        };
        // No reset_for_new_request call.
        let res = adapter.transfer_flat_to_paged(&[None, None], 0, &[]);
        assert!(res.is_err(), "expected error before reset_for_new_request");
        let msg = res.err().unwrap();
        assert!(
            msg.contains("reset_for_new_request"),
            "error must reference reset_for_new_request, got: {msg}"
        );
    }

    /// Reject calls after find_cached_prefix / record_tokens (adapter
    /// must be in a freshly-reset state).
    #[test]
    fn test_transfer_flat_to_paged_rejects_dirty_adapter() {
        let allocator = new_allocator(8, 16);
        let Some(mut adapter) = maybe_adapter(allocator, 16) else {
            eprintln!(
                "skipping test_transfer_flat_to_paged_rejects_dirty_adapter: \
                 Metal device unavailable"
            );
            return;
        };
        adapter.reset_for_new_request(0).unwrap();
        adapter.allocate_suffix_blocks(16).unwrap();
        adapter.record_tokens(&[1, 2, 3]).unwrap();
        // Now adapter has request_tokens=[1,2,3] — not freshly-reset.
        let res = adapter.transfer_flat_to_paged(&[None, None], 3, &[1, 2, 3]);
        assert!(res.is_err(), "expected error on dirty adapter");
        let msg = res.err().unwrap();
        assert!(
            msg.contains("freshly-reset"),
            "error must reference freshly-reset state, got: {msg}"
        );
    }

    /// Reject mismatched request_tokens / num_tokens.
    #[test]
    fn test_transfer_flat_to_paged_rejects_token_count_mismatch() {
        let allocator = new_allocator(8, 16);
        let Some(mut adapter) = maybe_adapter(allocator, 16) else {
            eprintln!(
                "skipping test_transfer_flat_to_paged_rejects_token_count_mismatch: \
                 Metal device unavailable"
            );
            return;
        };
        adapter.reset_for_new_request(0).unwrap();
        let res = adapter.transfer_flat_to_paged(&[None, None], 5, &[1, 2, 3]);
        assert!(res.is_err(), "expected error on token count mismatch");
        let msg = res.err().unwrap();
        assert!(
            msg.contains("request_tokens.len()"),
            "error must reference token mismatch, got: {msg}"
        );
    }

    /// Reject mismatched layer_caches / pool layer count.
    #[test]
    fn test_transfer_flat_to_paged_rejects_layer_count_mismatch() {
        let allocator = new_allocator(8, 16);
        let Some(mut adapter) = maybe_adapter(allocator, 16) else {
            eprintln!(
                "skipping test_transfer_flat_to_paged_rejects_layer_count_mismatch: \
                 Metal device unavailable"
            );
            return;
        };
        adapter.reset_for_new_request(0).unwrap();
        // Pool has 2 layers; pass 3.
        let res = adapter.transfer_flat_to_paged(&[None, None, None], 0, &[]);
        assert!(res.is_err(), "expected error on layer count mismatch");
        let msg = res.err().unwrap();
        assert!(
            msg.contains("flat_layer_kv.len()"),
            "error must reference flat_layer_kv length mismatch, got: {msg}"
        );
    }

    /// Zero-token transfer: no-op, leaves adapter in freshly-reset state.
    #[test]
    fn test_transfer_flat_to_paged_zero_tokens_is_noop() {
        let allocator = new_allocator(8, 16);
        let Some(mut adapter) = maybe_adapter(allocator, 16) else {
            eprintln!(
                "skipping test_transfer_flat_to_paged_zero_tokens_is_noop: \
                 Metal device unavailable"
            );
            return;
        };
        adapter.reset_for_new_request(0).unwrap();
        adapter
            .transfer_flat_to_paged(&[None, None], 0, &[])
            .expect("zero-token transfer must succeed");
        // Adapter still freshly-reset; another transfer call must NOT
        // hit the freshly-reset rejection path (request_tokens still
        // empty).
        adapter
            .transfer_flat_to_paged(&[None, None], 0, &[])
            .expect("a second zero-token transfer must succeed");
        assert_eq!(adapter.current_token_count(), 0);
    }

    /// Happy-path: BF16 transfer of a 16-token slab into paged pool
    /// (one block). Skips cleanly on no-Metal hosts.
    #[cfg(target_os = "macos")]
    #[test]
    fn test_transfer_flat_to_paged_writes_succeed_on_metal_bf16() {
        let cfg = mlx_paged_attn::PagedAttentionConfig {
            block_size: 16,
            num_kv_heads: 2,
            head_size: 64,
        ...
        let pool = match mlx_paged_attn::LayerKVPool::new(
            ...
            Ok(p) => Arc::new(p),
            Err(e) => {
                eprintln!("Skipping test_transfer_flat_to_paged_writes_succeed_on_metal_bf16: {e}");
                return;
            }
        };
        let allocator = Arc::new(Mutex::new(BlockAllocator::new(4, 16)));
        let mut adapter = PagedKVCacheAdapter::new(allocator, pool, 16).expect("adapter");
        adapter.reset_for_new_request(7).unwrap();

        // Build flat slabs in [B=1, num_kv_heads=2, num_tokens=16, head_dim=64]
        // BF16 layout — same as Qwen3.5 production caches.
        let num_tokens: usize = 16;
        let num_kv_heads: usize = 2;
        let head_dim: usize = 64;
        // [B=1] dimension factored out — total = B * H * T * D, B=1.
        let total = num_kv_heads * num_tokens * head_dim;
        // Construct dummy values (zeros) — only correctness of layout / pool
        // layout / dispatch matters here; the parity test (which needs
        // real model weights) covers numeric correctness.
        ...
        let k0 = MxArray::from_bfloat16(&zeros, &shape).expect("k0");
        let v0 = MxArray::from_bfloat16(&zeros, &shape).expect("v0");
        let k1 = MxArray::from_bfloat16(&zeros, &shape).expect("k1");
        let v1 = MxArray::from_bfloat16(&zeros, &shape).expect("v1");
        k0.eval();
        v0.eval();
        k1.eval();
        v1.eval();

        // Pass `Some` for both layers (no Linear-attention layers in this
        // synthetic config). The helper dispatches `write_kv` once per
        // layer; on success, request_tokens / num_tokens are seeded.
        let request_tokens: Vec<u32> = (0..num_tokens as u32).collect();
        let layer_kvs = vec![Some((&k0, &v0)), Some((&k1, &v1))];
        ...
        match res {
            ...
            Err(e) => {
                ...
                assert!(
                    e.contains("Metal GPU not available"),
                    "unexpected error from transfer_flat_to_paged (BF16): {e}"
                );
                return;
            }
        }

        // Post-conditions: adapter has 16 recorded tokens and 1 allocated
        // block.
