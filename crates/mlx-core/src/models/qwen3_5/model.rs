...
impl Qwen35Inner {
    ...
    /// ([`super::gdn_sidecar::capture_gdn_cold_sidecar`]); one sidecar is
    /// ~75 MiB on the 27B.
    fn capture_dense_gdn_cold_sidecar(
        ...
        super::gdn_sidecar::capture_gdn_cold_sidecar(
            "qwen3_5",
            self.paged_adapter.as_ref(),
            &self.gdn_prefix_checkpoints,
            &self.active_cache_owner_id,
...
mod eval_teacher_forced_tests {
    ...
    fn test_dense_scoring_a_checkpoint_against_its_own_capture_is_exactly_zero() {
        if let Err(err) = run_dense_self_identity() {
            ...
            if msg.contains("Metal") || msg.contains("device") {
                eprintln!(
                    "skipping test_dense_scoring_a_checkpoint_against_its_own_capture: {msg}"
                );
                return;
            }
