...
pub(crate) fn build_draft_lm_head(target: Option<&LinearProj>) -> Result<Option<LinearProj>> {
    ...
    let (dense, bias) = match proj {
        ...
        LinearProj::Quantized(ql) => (ql.dequantize_bf16()?, ql.additive_bias().cloned()),
    };
    let (packed, scales, biases) = quantize_affine(&dense, group_size, bits)?;
    Ok(Some(LinearProj::Quantized(QuantizedLinear::new(
        packed,
        scales,
        Some(biases),
        bias,
...
fn quantize_affine(
    ...
    Ok((
        MxArray::from_handle(out_q, "dflash2 quantized weight")?,
        MxArray::from_handle(out_s, "dflash2 quantization scales")?,
        MxArray::from_handle(out_b, "dflash2 quantization biases")?,
    ))
}
