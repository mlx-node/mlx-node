    ...
    let (thread, init_rx) = crate::model_thread::ModelThread::spawn_with_init(
        move || {
            ...
            let load_result: Result<(Qwen35Inner, u64)> = (|| -> Result<(Qwen35Inner, u64)> {
                ...
                // only now allocate/cap the physical paged KV pool. Shadowing the
                // witness (rather than reassigning) keeps the cold-tier gate below
                // the LAST materialize pass, not merely the first.
