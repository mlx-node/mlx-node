...
impl QuantizedLinear {
    ...
    /// `M >= 3` → W8A8 prefill. Default routing consumes only `self.weight`
    /// `[N,K]`; `MLX_SYM8_DUAL_LAYOUT=1` (read at load) retains `[K,N]` and
    /// restores the exact former wrappers, including their diagnostic env
    /// switches. Fail-loud on kernel error (there is no affine pack fallback).
...
mod sym8_tests {
    ...
    /// The default owns only `[N,K]`; the explicit compatibility policy owns
    /// one additional, bit-exact `[K,N]` transpose and preserves old outputs.
    #[test]
    fn sym8_dual_layout_fallback_residency_and_parity() {
        ...
        let (n, k) = (48i64, 64i64);
        let params = synth_sym8_params("fallback", n, k, 0x5197_1001);
        ...
        let stored = dual_nk.to_int8().unwrap();
        let transposed = dual_kn.to_int8().unwrap();
        for ni in 0..n as usize {
            for ki in 0..k as usize {
                assert_eq!(
                    stored[ni * k as usize + ki],
                    transposed[ki * n as usize + ni],
                    "fallback transpose mismatch at (n={ni}, k={ki})"
                );
            }
        }
        ...
        for (m, seed) in [
            ...
            assert_bf16_bit_identical(
                ...
                &format!("single/dual M={m}"),
            );
        }
    }

    /// Fresh-process gate for the public loader's environment wiring. The
    /// same test is run once with the variable unset and once with `=1`.
    #[test]
    fn sym8_loader_policy_matches_process_env() {
        ...
        let params = synth_sym8_params("env_policy", 16, 32, 0x5197_2001);
