...
impl Qwen3_5VisionEncoder {
    ...
    pub fn forward(&self, hidden_states: &MxArray, grid_thw: &MxArray) -> Result<MxArray> {
        ...
        // Forward through encoder layers. Each layer's attention builds its
        // additive mask internally from `cu_seqlens` (identical for every
        // layer, since LayerNorm preserves dtype and no layer changes the
        // token count).
