...
/// Eager paged MoE MTP Step-A forward: a single `[1, 1]` paged forward
/// returning both the verifier logits and the post-`final_norm` hidden.
///
/// Routes full-attention layers through the paged adapter (writing one new K/V
/// slot into the pool, attending over the full context) and GDN layers through
/// the flat `Qwen3_5LayerCache::Linear` slots in `caches`, the same split
/// [`run_paged_decode_step`] uses.
///
/// Returns `(logits [1, 1, vocab], hidden [1, hidden])`. The hidden is squeezed
/// on the time axis to match the `MtpStepper::forward_with_hidden` contract
/// (`needs_squeeze = true`).
#[allow(clippy::too_many_arguments)]
...
/// Eager paged MoE MTP batched verify forward: a single `[1, K+1]` paged
/// forward returning the verifier target distribution and the
/// post-`final_norm` hidden at every verify position, recording the per-layer
/// GDN tape for the rollback replay.
///
/// The `verify_ids` (`[1, K+1]` int32) are recorded into the adapter in ONE
...
/// K+1 query positions over the full context), GDN via the flat `Linear` slots
/// while recording a `GdnLayerTape` (the bit-exactness keystone the rollback
/// replay consumes). The `[1, K+1, H]` expert tail is the same
/// `SparseMoeBlock::forward` a prefill chunk drives — routing flattens the
/// time axis before dispatch, so no verify-specific expert path exists.
///
/// The `tape` is pre-sized / cleared by this function to `layers.len()`
