...
/// Parse a generated suffix whose prompt may already have emitted
/// `<|channel>thought\n`. In that case the generated bytes begin with the
/// reasoning BODY, not another channel opener, so seed the parser inside the
/// channel and treat the first `<channel|>` as its close.
pub(crate) fn parse_gemma4_output_with_open_channel(
    text: &str,
    starts_in_open_channel: bool,
) -> Gemma4ParsedOutput {
    let mut parser = Gemma4StreamParser::new_with_open_channel(starts_in_open_channel);
...
impl Gemma4StreamParser {
    pub(crate) fn new() -> Self {
        Self::new_with_open_channel(false)
    }

    pub(crate) fn new_with_open_channel(starts_in_open_channel: bool) -> Self {
        Self {
            state: if starts_in_open_channel {
                StreamState::Channel
            } else {
                StreamState::Message
            },
            pending: String::new(),
            thinking: String::new(),
            saw_channel: starts_in_open_channel,
            // The `thought\n` label is in the PROMPT for this seeded case,
            // so generated output starts at the body and must not be stripped.
...
mod tests {
    ...
    #[test]
    fn parse_prompt_opened_channel_keeps_reasoning_out_of_visible_text() {
        let parsed =
            parse_gemma4_output_with_open_channel("plan the answer<channel|>final answer", true);
        assert_eq!(parsed.thinking.as_deref(), Some("plan the answer"));
        assert_eq!(parsed.text, "final answer");

        let ordinary =
            parse_gemma4_output_with_open_channel("plan the answer<channel|>final answer", false);
        assert!(ordinary.thinking.is_none());
        assert_eq!(ordinary.text, "plan the answerfinal answer");
    }
    ...
    #[test]
    fn stream_parser_prompt_opened_channel_routes_body_as_reasoning() {
        let mut parser = Gemma4StreamParser::new_with_open_channel(true);
        let mut all = parser.feed("plan");
        all.extend(parser.feed(" carefully<chan"));
        all.extend(parser.feed("nel|>answer"));
        all.extend(parser.flush());

        let reasoning: String = all
            .iter()
            .filter_map(|segment| match segment {
                StreamSegment::Reasoning(text) => Some(text.as_str()),
                _ => None,
            })
            .collect();
        let text: String = all
            .iter()
            .filter_map(|segment| match segment {
                StreamSegment::Text(text) => Some(text.as_str()),
                _ => None,
            })
            .collect();
        assert_eq!(reasoning, "plan carefully");
        assert_eq!(text, "answer");
        assert_eq!(parser.thinking().as_deref(), Some("plan carefully"));
    }
