...
fn assistant_message(result: &ChatResult) -> ChatMessage {
    ChatMessage {
        ...
        // LFM2's checkpoint template reads the complete historical
        // `<think>...</think>` block from `content` and ignores the
        // structured `reasoning_content` field used by Qwen/Gemma. Preserve
        // the exact generated bytes so the next full-history render strictly
