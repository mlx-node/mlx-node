...
//!
//! NemotronH is a hybrid Mamba-2 SSM + GQA + pure MoE-FFN architecture: only
//! the six `full_attention` layers route through the block-paged adapter;
//! mamba layers keep per-request recurrent state and moe layers are
//! stateless. Greedy (T=0) parity over real weights is the gate that proves
...
//!
//! ```shell
//! MLX_TEST_MODEL_PATH=/abs/nemotron-h \
//!     cargo test -p mlx-core --test nemotron_h_paged_vs_flat_parity \
//!     -- --ignored --nocapture
//! ```
...
/// Copy the source NemotronH checkpoint directory into a fresh tempdir under
/// the workspace `target/` and patch `config.json` to pin the paged flag.
fn clone_model_dir(src: &Path, suffix: &str, use_block_paged: bool) -> Result<PathBuf, String> {
    ...
    // would silently default-on and compare paged-vs-paged.
    ...
    if use_block_paged {
        // 6 attention layers x (32 Q / 2 KV heads x head_dim 128) at 16-token
        // blocks; 512 MiB comfortably holds the test's short prompts.
...
fn assistant_message(result: &ChatResult) -> ChatMessage {
    ChatMessage {
        ...
        // Preserve the exact generated bytes so the next full-history render
        // strictly extends the live cache.
...
/// Two-turn dialog parity: validates `find_cached_prefix` +
/// `finalize_turn_keep_live` cross-turn semantics on the paged side against
