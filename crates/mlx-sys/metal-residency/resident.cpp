...
namespace {
int residency_set_max_pct() {
  static int pct = env::get_var("MLX_RESIDENCY_SET_MAX_PCT", 5);
  return pct;
}
bool residency_debug() {
  static bool debug = env::get_var("MLX_RESIDENCY_DEBUG", 0);
  return debug;
}
} // namespace
...
uint32_t ResidencySet::choose_set_locked(size_t bytes) {
...
uint32_t ResidencySet::add_to_set_locked(
...
void ResidencySet::remove_from_set_locked(
    const MTL::Allocation* buf,
    Placement& at) {
...
void ResidencySet::attach_new_sets(MTL::CommandQueue* q, uint64_t& attached) {
