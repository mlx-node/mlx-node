//! Coexistence smoke test for the Qwen3.5 MoE per-model compiled slot.
//!
//! Proves that two MoE models (distinct `model_id`) each keep their OWN
...
//! demoted the 1st to the eager path). Without the per-model `MoeSlot` map, the
//! two models would share the one set of file-scope working-register globals and
//! clobber each other. Mirrors the dense `qwen3_5_coexist_smoke.rs`.
//!
...
//! stamp a sentinel into each model's slice of `g_paged_linear_caches`:
//!
...
unsafe extern "C" {
    fn mlx_qwen35_activate_moe_model(model_id: u64);
    fn mlx_qwen35_moe_model_has_weights(model_id: u64) -> i32;
    fn mlx_qwen35_moe_slot_erase(model_id: u64);
    fn mlx_qwen35_moe_compiled_test_force_paged_linear_caches(
        num_layers: i32,
        full_attention_interval: i32,
    );
    fn mlx_qwen35_moe_compiled_test_read_paged_linear_slot(slot_idx: i32) -> f32;
    fn mlx_qwen35_moe_compiled_test_write_paged_linear_slot(slot_idx: i32, value: f32);
    fn mlx_qwen35_moe_reset();
    fn mlx_clear_weights(model_id: u64);
}

#[test]
fn moe_compiled_slots_isolate_two_models() {
    if !metal_available() {
        eprintln!("skipping moe_compiled_slots_isolate_two_models: Metal unavailable on this host");
        return;
    }

    unsafe {
        // Clean slate: drop any slots a prior run in this process left behind
        // and clear the working register.
        mlx_qwen35_moe_slot_erase(MODEL_A);
        mlx_qwen35_moe_slot_erase(MODEL_B);
        mlx_qwen35_moe_reset();

        // Model A becomes the working register; stamp its slot-0 sentinel.
        mlx_qwen35_activate_moe_model(MODEL_A);
        mlx_qwen35_moe_compiled_test_force_paged_linear_caches(NUM_LAYERS, FULL_ATTN_INTERVAL);
        mlx_qwen35_moe_compiled_test_write_paged_linear_slot(0, SENTINEL_A);
        assert_eq!(
            mlx_qwen35_moe_compiled_test_read_paged_linear_slot(0),
            SENTINEL_A,
        ...
        mlx_qwen35_activate_moe_model(MODEL_B);
        mlx_qwen35_moe_compiled_test_force_paged_linear_caches(NUM_LAYERS, FULL_ATTN_INTERVAL);
        mlx_qwen35_moe_compiled_test_write_paged_linear_slot(0, SENTINEL_B);
        assert_eq!(
            mlx_qwen35_moe_compiled_test_read_paged_linear_slot(0),
            SENTINEL_B,
        ...
        assert_eq!(
            mlx_qwen35_moe_model_has_weights(MODEL_A),
            0,
            "has_weights must be false for an unregistered model"
        );
        assert_eq!(mlx_qwen35_moe_model_has_weights(MODEL_B), 0);
        ...
        // was PARKED — exactly what the MoE loader now does (via
        // `mlx_qwen35_moe_slot_erase(model_id)`) for the model it is
        // (re-)registering, because `mlx_qwen35_moe_invalidate_compiled_graphs()`
        // only reaches the working register and would otherwise leave a parked
        // model's stale baked graphs intact. Re-activating A must therefore yield
        // a FRESH, empty slot (its forced caches are gone → out-of-range read →
        // NaN), proving the stale SENTINEL_A did NOT survive the erase. A real
        // same-id reload then re-traces against the newly stored weights instead
        // of swapping the old tape back in.
        mlx_qwen35_activate_moe_model(MODEL_A);
        assert!(
            mlx_qwen35_moe_compiled_test_read_paged_linear_slot(0).is_nan(),
