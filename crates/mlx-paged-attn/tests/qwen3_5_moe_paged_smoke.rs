...
//!    pool / scale arrays, calling `mlx_qwen35_moe_forward_paged` does
...
const NUM_EXPERTS_PER_TOK: i32 = 8;
const NORM_TOPK: i32 = 1;
const DECODER_SPARSE_STEP: i32 = 1;
...
/// crash. This proves:
///
///   1. The new globals (`g_k_pools`, `g_v_pools`, `g_k_scales`,
///      `g_v_scales`, `g_paged_linear_caches`) are wired up.
///   2. `mlx_qwen35_moe_init_paged` accepts a complete per-layer
///      handle bundle without tripping any validation guard.
///   3. `mlx_qwen35_moe_forward_paged` reaches the compile-graph
...
fn forward_paged_graph_builds_without_crash() {
    ...
    // Cleanup all input handles. The pool / scale handles are held by
    // the C++ globals as MLX `array` copies (refcount bump on copy);
    // dropping the test's pointers via `mlx_array_delete` is safe.
    unsafe {
        delete(input_ids);
        delete(embedding_weight);
        delete(offset_arr);
