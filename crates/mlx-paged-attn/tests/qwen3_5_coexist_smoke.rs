//! Coexistence smoke test for the Dense Qwen3.5 per-model compiled slot.
//!
//! Proves that two dense models (distinct `model_id`) each keep their OWN
...
//! demoted the 1st to the eager path). Without the per-model `DenseSlot` map,
//! the two models would share the one set of file-scope working-register
//! globals and clobber each other.
//!
...
//! stamp a sentinel into each model's slice of `g_dense_paged_linear_caches`:
//!
...
unsafe extern "C" {
    fn mlx_qwen35_activate_dense_model(model_id: u64);
    fn mlx_qwen35_model_has_weights(model_id: u64) -> i32;
    fn mlx_qwen35_dense_slot_erase(model_id: u64);
    fn mlx_qwen35_compiled_test_force_paged_linear_caches(
        num_layers: i32,
        full_attention_interval: i32,
    );
    fn mlx_qwen35_compiled_test_read_paged_linear_slot(slot_idx: i32) -> f32;
    fn mlx_qwen35_compiled_test_write_paged_linear_slot(slot_idx: i32, value: f32);
    fn mlx_qwen35_compiled_reset();
    fn mlx_clear_weights(model_id: u64);
}

#[test]
fn dense_compiled_slots_isolate_two_models() {
    if !metal_available() {
        eprintln!(
            "skipping dense_compiled_slots_isolate_two_models: Metal unavailable on this host"
        );
        return;
    }
    ...
    unsafe {
        ...
        mlx_qwen35_dense_slot_erase(MODEL_A);
        mlx_qwen35_dense_slot_erase(MODEL_B);
        mlx_qwen35_compiled_reset();

        // Model A becomes the working register; stamp its slot-0 sentinel.
        mlx_qwen35_activate_dense_model(MODEL_A);
        mlx_qwen35_compiled_test_force_paged_linear_caches(NUM_LAYERS, FULL_ATTN_INTERVAL);
        mlx_qwen35_compiled_test_write_paged_linear_slot(0, SENTINEL_A);
        assert_eq!(
            mlx_qwen35_compiled_test_read_paged_linear_slot(0),
            SENTINEL_A,
        ...
        mlx_qwen35_activate_dense_model(MODEL_B);
        mlx_qwen35_compiled_test_force_paged_linear_caches(NUM_LAYERS, FULL_ATTN_INTERVAL);
        mlx_qwen35_compiled_test_write_paged_linear_slot(0, SENTINEL_B);
        assert_eq!(
            mlx_qwen35_compiled_test_read_paged_linear_slot(0),
            SENTINEL_B,
        ...
        assert_eq!(
            mlx_qwen35_model_has_weights(MODEL_A),
            0,
            "has_weights must be false for an unregistered model"
        );
        assert_eq!(mlx_qwen35_model_has_weights(MODEL_B), 0);
        ...
        // was PARKED — exactly what the dense loader now does (via
        // `mlx_qwen35_dense_slot_erase(model_id)`) for the model it is
        // (re-)registering, because `invalidate_compiled_graphs()` only reaches
        // the working register and would otherwise leave a parked model's stale
        // baked graphs intact. Re-activating A must therefore yield a FRESH,
        // empty slot (its forced caches are gone → out-of-range read → NaN),
        // proving the stale SENTINEL_A did NOT survive the erase. A real
        // same-id reload then re-traces against the newly stored weights
        // instead of swapping the old tape back in.
        mlx_qwen35_activate_dense_model(MODEL_A);
        assert!(
            mlx_qwen35_compiled_test_read_paged_linear_slot(0).is_nan(),
