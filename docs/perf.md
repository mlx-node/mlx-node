...
`false` / `off`, case-insensitive, with `trim()`). The adaptive-depth knob
...
| ----------------------------- | -------------------- | ---------- | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~`MLX_MTP_USE_TAPE_REPLAY`~~  | ~~ON~~               | W6.6       | ~~opt-OUT~~     | **REMOVED (dead).** The var is unread; the GDN tape replay (restore-and-replay on partial accept) is unconditional today. |
...
| `MLX_MTP_DEFER_VERIFY_HIDDEN` | ON                   | W6.27      | opt-OUT       | Keeps `verify_hiddens` lazy after the verify dispatch (`async_eval` only the logits); the chained-cycle seed slice materializes at the next-cycle boundary via `eval_step_with_chained_hidden`. Set to `0` / `false` / `off` to async-eval logits + hiddens together. |
...
| `MLX_MTP_SPARSE_ACCEPT`       | ON                   | W6.19      | opt-OUT       | Batched argmax over D+1 verify positions at T=0 with no penalties; collapses D × full-vocab softmax materializations into one .eval(). Falls back to legacy per-position path at T>0 or when sampling penalties are active. Set to `0` / `false` / `off` to revert to the per-position path. |
...
| `MLX_MTP_ACCEPT_GATE`         | ON                   | —          | opt-OUT       | MTP acceptance gate, **depth-1-scoped, adaptive-exempt, and confidence-aware**. When ON (default), the model aggregates the first-draft acceptance counts (accepted/attempted at draft slot 0) across completed depth-1 turns and disables speculation for the NEXT depth-1 turn only when the 95% Wilson confidence bound on the aggregate rate is below `0.6` AND at least 4 drafts have been sampled — plain AR — so a weak draft head does not pay the verify cost for zero speedup. The confidence bound + minimum sample make undersampling harmless: a 1-cycle turn (exactly 0.0 or 1.0), a 2-of-4 streak from a healthy 0.756 head (~25% of turns, upper bound ~0.82), and a 0-of-2 or 0-of-3 aggregate (~5.95% / ~1.45% false rates) never gate; only 0-of-4+ (false rate ~0.35%) does. The aggregate is bounded (halved past ~512 samples) so a long healthy phase cannot drown out a later degradation. Depth > 1 turns and `mtpAdaptiveDepth` turns (which sweep depths 1-5) are never gated and do not publish history. State is per-model; the first turn of a model load probes, after 3 consecutive gated turns the gate re-probes, and a full session reset (`reset_caches`) clears the history. Measured first-draft acceptance on real checkpoints: 1.000 (qwen3.5-4b, counting), 0.756 (qwen3.5-4b, complex task), 0.000 (qwen3.5-0.8b — MTP is a ~0.6× net loss there). Set to `0` / `false` / `off` to always run MTP when requested. |
...
- (The former `MLX_MTP_USE_TAPE_REPLAY=0` K+1-replay opt-out is gone — the var is unread; the tape replay is unconditional.)
...
- Source of truth (env-var readers + inventory table):
  ...
  `mtp_sparse_accept_enabled`, `mtp_no_prompt_prefill`,
  `mtp_defer_verify_hidden_eval`). The W6.29 bucketed-verify dispatcher no
  longer exists — it was C++-side state in the deleted `mlx_qwen35.cpp`.
  (`MLX_MTP_USE_TAPE_REPLAY` is unread today — the tape replay is
  unconditional on partial accept.)
