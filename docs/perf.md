...
| `MLX_MTP_ACCEPT_GATE`         | ON                   | —          | opt-OUT       | MTP acceptance gate. When ON (default), a completed MTP turn whose FIRST-draft acceptance rate (per-position acceptance at draft slot 0 — depth-agnostic) fell below `0.6` disables speculation for the NEXT turn — plain AR — so a weak draft head does not pay the verify cost for zero speedup. (At depth > 1 the first-draft rate is used instead of the accepted/attempted average, which the depth-1 threshold would misclassify.) State is per-model; the first turn of a model load probes, after 3 consecutive gated turns the gate re-probes, and a full session reset (`reset_caches`) clears the history. Measured first-draft acceptance on real checkpoints: 1.000 (qwen3.5-4b, counting), 0.756 (qwen3.5-4b, complex task), 0.000 (qwen3.5-0.8b — MTP is a ~0.6× net loss there). Set to `0` / `false` / `off` to always run MTP when requested. |
