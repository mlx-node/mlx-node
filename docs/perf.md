...
| `MLX_MTP_ACCEPT_GATE`         | ON                   | —          | opt-OUT       | MTP acceptance gate. When ON (default), a completed MTP turn whose draft-acceptance ratio (accepted/attempted drafts) fell below `0.6` disables speculation for the NEXT turn — plain AR — so a weak draft head does not pay the verify cost for zero speedup. State is per-model (shared across ChatSessions over one load); the first turn of a model load probes, and after 3 consecutive gated turns the gate re-probes so a later easier turn can re-enable MTP. Measured on real checkpoints: 1.000 (qwen3.5-4b, counting), 0.756 (qwen3.5-4b, complex task), 0.000 (qwen3.5-0.8b — MTP is a ~0.6× net loss there). Threshold is depth-1-tuned. Set to `0` / `false` / `off` to always run MTP when requested. |
