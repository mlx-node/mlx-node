...
| `MLX_KQUANT_SMALL_M_BENCH=1`  | Un-ignores the `kquant_small_m_bench` integration test: exact-shape K/IQ small-M quantized-matmul timings (MLP + lm_head shapes, M sweep). |
...
| `MLX_DISABLE_QATTN_KV_MERGE=1` | Restore two separate quantized matmuls for attention k\|v pairs where the merge applies (checked per forward, so it can flip without reload). Bit-identical; A/B and rollback. Note attention projections carrying an activation-calibration amax key are never merged — those sites already keep separate projections. |
