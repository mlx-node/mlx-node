...
| `MLX_MTP_ACCEPT_GATE`         | ON                   | —          | opt-OUT       | MTP acceptance gate, **depth-1-scoped**. When ON (default), a completed depth-1 MTP turn with at least 4 speculative cycles whose FIRST-draft acceptance rate (per-position acceptance at draft slot 0) fell below `0.6` disables speculation for the NEXT depth-1 turn — plain AR — so a weak draft head does not pay the verify cost for zero speedup. Undersampled turns (< 4 cycles) do not publish: a 1-cycle turn records exactly 0.0 or 1.0, which is not representative (a 0.756-acceptance head would wrongly record 0.0 ~24% of the time). Depth > 1 turns are never gated and do not publish history. State is per-model; the first turn of a model load probes, after 3 consecutive gated turns the gate re-probes, and a full session reset (`reset_caches`) clears the history. Measured first-draft acceptance on real checkpoints: 1.000 (qwen3.5-4b, counting), 0.756 (qwen3.5-4b, complex task), 0.000 (qwen3.5-0.8b — MTP is a ~0.6× net loss there). Set to `0` / `false` / `off` to always run MTP when requested. |
