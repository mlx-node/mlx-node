...
| Muse-Glimmer  | none                                                         | fresh and delta, via the gemma4-shaped grouped full/sliding coordinator; the pools are hidden while a flat DFlash owner is live | DFlash on flat target state by default; opt-in packed paged verification with `MLX_SCHEDULED_DFLASH=1`, streaming-capable                                                                          | DFlash uses the barrier lane by default; the packed route remains experimental                                                                         |
