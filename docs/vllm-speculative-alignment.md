...
**Reopens if:** a GDN-bearing _drafter_ ever appears, or the pool residency cliff is eliminated.
...
**Why:** we emit tokens _inside_ the cycle as they are accepted, for streaming latency. vLLM emits at step
...
**Us:** `run_dspark_turn` clamps the accepted run at the stop token _before_ committing, so the stop token's
...
**Us:** nothing to port — we get this by construction. Our verify runs _inside_ the family's grouped layer
...
diverges between paged MTP and paged AR — and equally between _flat_ MTP and _flat_ AR, and even between paged
