...
| Stage | What | Gate |
|---|---|---|
| ~~**D1**~~ | **Landed.** gemma4 DSpark verifies against the paged pools through `SpecPagedCache`; `SpecTurnEpilogue` makes L-EPILOGUE executable (an abandoned epilogue is counted and debug-asserts). The flat lane narrowed to the assistant drafter, whose Q-only attention reads flat `Gemma4LayerCache` K/V directly — that is D4. | 2.1× (free-form, 200 tok) to 4.5× (constrained, >1100-tok prompt) vs paged AR, same binary; T=0 byte parity on both sliding-wrap legs |
