...
The checkpoint SHA-256 is `93567e57a8fe10b23569b9d9ec38cd005deedf71e29477c421a4b83f418a538b`. llama.cpp is pinned to `a14dba686aaafba3a2d6b5eb8820b0df5c5d2d92`, and MLX to `6d45ab90cfec5e7fe0cfe25bc635a23ac8a351bb`. The tested native addon SHA-256 is `2f380276a3fa2e3dc39f3e5ff377bc213152554d34d2ba3d819db733d3adface`. [All samples and provenance](./results.json) contain min/max values, timing accounting, memory snapshots, generated-output hashes, and the selected plans. The source patch includes both tracked modifications and new source files; its hash is recorded alongside individual file hashes. The recorded hashes retain the measured source. Subsequent changes reordered the `decode_tuning` and `decoder_layer` declarations for rustfmt and corrected GGUF media preflight to reuse the runtime's unified-media capabilities, with regression coverage for plain Gemma legacy audio and unified vision/audio. These changes do not alter the timed inference path; the benchmark matrix was not rerun for them.
