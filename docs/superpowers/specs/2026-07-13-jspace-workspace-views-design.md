...
3. **Motor-flip crossover marker** — per prompt position, the layer at which the readout's top guess first locks onto that position's final (ℓ-max) token and stays there. The "motor" boundary from the paper's sensory/workspace/motor three-block picture. Pure client-side, works baked and live.
