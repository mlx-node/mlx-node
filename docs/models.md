...
**So the reason the default stays off is concurrency, not a throughput loss.** `enable_mtp == Some(true)` trips `chat_requires_barrier` in the hybrid scheduler (`crates/mlx-core/src/engine/hybrid_scheduler.rs`), which moves the turn into the **exclusive** lane and out of continuous batching entirely; streaming MTP turns fall back to paged AR on top of that (`mtp_flat_routing_required`). Auto-enabling would trade every session's batching for a speedup that measures as noise. Turning it on for a single un-batched session is a fair choice and now costs nothing measurable — that is the part the old number got wrong.
