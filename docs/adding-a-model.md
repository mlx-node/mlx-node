...
**Command plumbing.** Use the engine-owned
`ModelCommand<FamilyCommand>` (`engine/model_command.rs`). It carries boxed
`ChatCmd`, scheduler telemetry, and a typed family extension. Inference-only
families use `ModelCommand` directly: its default extension is `Infallible`,
so they declare no command enum, conversion implementation, or adapter macro.

A family with additional operations defines only those operations in its enum
and implements `FamilyCommand<FamilyInner>::execute`. `ModelThread::send` and
`send_and_await` accept that enum through the shared `From` implementation.
Trainable families implement `FromTrainCmd` on their extension enum; the envelope
inherits that conversion. All extensions remain ordered barriers.

Keep shared behavior in backend trait defaults or generic engine functions.
`HybridSchedulerBackend` supplies normal chat-barrier dispatch; override
`execute_chat_barrier` only for additional owner-state installation or
reconciliation. The engine handles chat construction/extraction, stats and
family dispatch once. No per-family enum adapter remains.
...
`ModelCommand`), `thread:` direct vs `Option` (gemma4 alone holds an
...
`chat_napi_surface!` using the shared command type, including Gemma4's load-first
...
- Implement **`HybridSchedulerBackend`** (`engine/hybrid_scheduler.rs`): 4
  associated types (`FamilyCommand`, `RestoreTicket`, `OwnerState`,
  `StepExecutor<'a>`), `const SCHEDULER_NAME`, the defaulted consts
  `ENABLED_BY_DEFAULT` / `CANCEL_PRECEDES_EOS` / `STREAM_EOS_TOKEN`, and 10
  ...
  `step_executor`. Barrier dispatch and the remaining policy hooks have defaults.
...
- Grouped-coordinator families (gemma4, muse_glimmer) also override
  ...
  custom `execute_chat_barrier` managing the flat-vs-paged owner lanes.
