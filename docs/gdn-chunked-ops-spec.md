...
⇒ S*t = g_t S*{t-1} + δ*t k_tᵀ, δ_t = β_t(v_t − g_t S*{t-1} k_t), y_t = S_t q_t.
...
- M = (I + A)⁻¹ = Σ*{m≥0} (−A)^m (A strictly-lower ⇒ nilpotent, exact). Compute via DOUBLING:
  N = −A; Q_0=N; M_0=I; for k in 0..6: M*{k+1}=M*k + Q_k@M_k; Q*{k+1}=Q_k@Q_k. M_6 covers m=0..63.
- WY (S_in-independent, precompute parallel over ALL chunks):
  u = M @ (β⊙v) [L,Dv]
  W = M @ ((β⊙decay_self)[:,None]⊙k) [L,Dk]
- SERIAL loop over chunks c (carry S*in_c [Dv,Dk], start = initial_state):
  ...
  stash S_in_c, δ_c ; S_in*{c+1}=S_out
