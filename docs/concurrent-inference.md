...
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Server    | One rolling-promise FIFO per model object; `/v1/responses` and `/v1/messages` share it (aliases too). Held through terminal SSE visibility. When `res.write()` returns false, endpoint consumption parks until drain or transport close/error; a slow live client can therefore extend lock hold time, while disconnect aborts and unwinds the turn.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `packages/server/src/session-registry.ts:1096-1137`, `streaming.ts:32-127`, `endpoints/responses.ts:361-420`, `endpoints/messages.ts:316-372` |
...
| H2  | Non-streaming requests cannot be cancelled — **fixed**         | The three synchronous chat commands carry a cancel flag and have additive `chatSession*Cancellable` NAPI twins. LM send paths accept `opts.signal`; both HTTP endpoints wire disconnects to an `AbortController`, cancel an active native call, and skip native dispatch when the client is already dead. Cancelled turns reject exactly with `"chat session cancelled"` and roll JS history back. (`models/chat_napi.rs:117-253`, `chat-session.ts:1675`, endpoint non-streaming dispatch paths.)                                                                                                  |
...
| H4  | SSE ignores backpressure — **fixed at the HTTP pull boundary** | `writeSSEEvent()` returns the real `res.write()` boolean. Both endpoints remember the first false return, stop pulling model events until drain, and settle safely on close/error; sticky abort tracking remains installed through residual and terminal drains. The live native `StreamTx`/JS producer queue is still unbounded while a peer stays connected but never drains, so this fix bounds HTTP-side buffering rather than claiming an end-to-end producer-memory cap. (`streaming.ts:32-127`, endpoint native streaming loops.)                                                            |
...
- **Stage 0 — robustness (landed):** H1–H4 are regression-locked: cancellable
  non-streaming chat turns + pre-dispatch disconnect checks, fail-closed
  chunk-boundary prefill cancellation, async reset, a default queue cap in
  host mode, and close-safe HTTP SSE backpressure. The remaining live-peer
  native producer-queue bound is stated explicitly above rather than hidden
  behind an RSS claim.
