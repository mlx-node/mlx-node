...
| Admission | Queue depth caps at 16 waiters by default (`DEFAULT_MAX_QUEUE_DEPTH_PER_MODEL`); over-cap requests get 429 + `Retry-After: 1`. Two gates share one budget: a pre-dispatch gate in both endpoints (`beginPreDispatchAdmission`, covers host-mode arrivals parked invisibly in the coordinator writer queue and continuations blocked in `store.getChain`) and the `withExclusive` waiter cap. The permit is retained through all pre-lock awaits and consumed atomically at `withExclusive` placement — one token per request, never double-counted; both gates charge the same combined footprint (`queuedCount + preDispatchAdmits`, runner slot outside the budget), so permitless cold-load dispatches cannot spend a slot an outstanding permit owns. Override via `maxQueueDepthPerModel` / `MLX_MAX_QUEUE_DEPTH_PER_MODEL`; config `'unbounded'` opts out. `createInferenceHost` inherits the default. | `session-registry.ts:614-661,1067-1093`, `server.ts:397-404`, `responses.ts:1789-1815`, `messages.ts:1069-1095` |
