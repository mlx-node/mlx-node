...
| H3 | Unbounded queue by default | No depth cap unless the env/config is set; `mlx serve` / `mlx launch claude` ship uncapped. Combined with H2, dead clients amplify backlog. |
...
| Admission / preemption | `max_num_seqs`, watermark, preempt-and-recompute | absent — unbounded FIFO by default |
