...
| **Gemma4**   |        Yes        |          Yes           |   off   | Forward dispatch wired (LFM2-style monolithic template + per-layer routing via `Gemma4LayerKind`). Sliding layers stay on flat `RotatingKVCache`; global layers route through the paged adapter; KV-shared layers consume the anchor's K/V via `SharedOnGlobal` (paged anchor) / `SharedOnSliding` (flat anchor stash). MoE/PLE/4-norm/layer-scalar reuse the existing `apply_ffn_ple_scalar` tail unchanged. Default stays `Some(false)` until `gemma4_paged_vs_flat_parity` confirms greedy + prefix-reuse parity on real weights. |
