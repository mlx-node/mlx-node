...
| **Gemma4**   |        Yes        |          Yes           |   off (parity-blocked)   | Forward dispatch wired (LFM2-style monolithic template + per-layer routing via `Gemma4LayerKind`). Sliding layers stay on flat `RotatingKVCache`; global layers route through the paged adapter; KV-shared layers consume the anchor's K/V via `SharedOnGlobal` (paged anchor) / `SharedOnSliding` (flat anchor stash). MoE/PLE/4-norm/layer-scalar reuse the existing `apply_ffn_ple_scalar` tail unchanged. **Known-WIP parity bug**: `gemma4_paged_vs_flat_parity` fails on real Gemma-4-E2B weights — token 0 of decode is correct (matches flat byte-equal) but tokens 1..N degrade into an `id, id+1, id+2, …` cycle, signature of attention contributing ~zero on KV-shared layers. Pool round-trip is bit-correct in isolation (`paged_pool_multi_block_round_trip` test passes for `head_dim=512` BF16 multi-block). Eliminated suspects: pool sync, slot mapping, position values, anchor-layer SDPA, SDPA backend, prefill chunking, eval-forcing on `update_keys_values`, full SharedOnGlobal stash refactor. Bug is somewhere on the SharedOn* paths and needs per-layer numerical-diff scaffolding (flat-vs-paged `hidden_states.norm()` dump per layer) to localize. Default stays `Some(false)` until parity is restored. |
| **Qwen3.5 Dense** |    Yes        |          Yes           |   off   | Forward dispatch wired via `Qwen3_5LayerKind` (`Linear` GDN / `FullAttentionPaged`). GDN linear-attention layers stay on flat `Qwen3_5LayerCache::Linear(ArraysCache)` and are reset+reprefilled every paged turn (no cross-request prefix reuse — vLLM `MambaManager` stance). Full-attention layers route through `Qwen3_5Attention::forward_paged`. **Compile lockout**: each chat-dispatch site (`chat_sync_core`, `chat_tokens_delta_sync`, `chat_stream_sync_inner`, `chat_stream_tokens_delta_sync_inner`) early-returns into `chat_sync_core_paged` / `chat_stream_sync_core_paged` BEFORE acquiring `DENSE_COMPILED_MUTEX`/`COMPILED_WEIGHTS_RWLOCK` and never calls `mlx_qwen35_compiled_init_from_prefill`, so flat-path turns and paged-path turns can interleave without corrupting compiled state. **VLM rejected**: `set_vision_encoder` errors when `paged_adapter.is_some()` (M-RoPE / vision-feature path requires bespoke paged plumbing, deferred). Default stays `Some(false)` until `qwen3_5_paged_vs_flat_parity` confirms parity on real weights. |
| **Qwen3.5 MoE**   |    Yes        |          Yes           |   off   | Same forward dispatch + compile-lockout + VLM-rejection as Qwen3.5 Dense; routes through MoE `DecoderLayer::forward_paged_or_flat` so the MoE/dense MLP variant stays unchanged. Default stays `Some(false)` until `qwen3_5_moe_paged_vs_flat_parity` confirms parity on real weights. |
...
Gemma4 and Qwen3.5 (Dense + MoE) stay default-off until parity is verified on real weights. Qwen3.5 forward dispatch is wired (this commit); the parity-test scaffolds in `crates/mlx-core/tests/qwen3_5_paged_vs_flat_parity.rs` and `crates/mlx-core/tests/qwen3_5_moe_paged_vs_flat_parity.rs` are gated on `MLX_TEST_MODEL_PATH` and `#[ignore]`-marked so they compile + skip cleanly without weights.
