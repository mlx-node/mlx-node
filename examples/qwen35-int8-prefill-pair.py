#!/usr/bin/env python3
"""
Qwen3.5 int8 W8A8 prefill A/B orchestrator — thermally-fair paired verdict.

Stage 4: measures the REALIZED prefill TTFT win from the MLP-only int8 path
(Stage 3) on the real bf16 Qwen3.5-4B, using the established lfm2 paired-
process methodology (median of per-pair ratios + a control noise band).

Launches the single-arm harness (examples/qwen35-int8-prefill-ab.ts)
alternately as the TREATMENT (MLX_INT8_PREFILL=1) and BASELINE
(MLX_INT8_PREFILL unset) arms, BACK TO BACK (adjacent processes share a
thermal window). Each pair yields one unit-free ratio; the median of per-pair
ratios cancels the ~15% cross-run thermal drift on M5 Max.

NOTE on toggle polarity (opposite of the lfm2 DISABLE-style harness):
  treatment = MLX_INT8_PREFILL=1     (int8 MLP prefill ON)
  baseline  = MLX_INT8_PREFILL unset (bf16 fused path, unchanged default)
The int8 weight quant happens at LOAD time, so each arm MUST be a fresh
process with the env set/unset — which the per-arm fork here guarantees.

A CONTROL set runs BOTH arms as BASELINE (identical code path) to measure
the ratio noise floor. An optimization counts as a REAL win only if its
median ratio improvement exceeds the control band AND the sign is consistent
across >=75% of pairs.

Ratio convention (>1.0 = treatment faster):
  ttft  -> prefill: medPrefillTps_treat / medPrefillTps_base  (higher better)
           ttft-ms: medTtftMs_base / medTtftMs_treat          (lower better)
           decode : medDecodeTps_treat / medDecodeTps_base    (~1.0 expected)
  decode-> medDecodeTps_treat / medDecodeTps_base

Usage:
  PATH=/usr/bin:$PATH python3 examples/qwen35-int8-prefill-pair.py \
    --model /Volumes/P4510/.cache/models/qwen3.5-4b --mode ttft \
...
HARNESS = os.path.join(os.path.dirname(os.path.abspath(__file__)), "qwen35-int8-prefill-ab.ts")
TOGGLE = "MLX_INT8_PREFILL"
# All int8-prefill toggles the treatment arm may control. The treatment arm sets
# exactly the subset named in --treatment-env (default: COMBINED = both MLP+qkvz);
# the baseline arm always UNSETS every one of these so it runs the bf16 default.
ALL_INT8_TOGGLES = ("MLX_INT8_PREFILL", "MLX_INT8_PREFILL_QKVZ")


def run_arm(args, treatment: bool) -> dict:
    """Run one harness arm. treatment=True -> sets args.treatment_env toggles;
    baseline always unsets ALL int8 toggles (bf16 default path)."""
    env = dict(os.environ)
    # Baseline must never inherit a stray int8 toggle from the parent env.
    for k in ALL_INT8_TOGGLES:
        env.pop(k, None)
    if treatment:
        for k in args.treatment_env:
            env[k] = "1"
    cmd = [
        "oxnode", HARNESS,
        "--model", args.model,
...
def metric(args, r: dict) -> float:
    # Primary verdict metric. For ttft mode the headline is prefill-tps;
    # we keep ttft-ms as a cross-check (computed separately).
...
def main():
    ...
    ap.add_argument("--model", default="/Volumes/P4510/.cache/models/qwen3.5-4b")
    ...
    ap.add_argument(
        "--treatment-env",
        default="MLX_INT8_PREFILL,MLX_INT8_PREFILL_QKVZ",
        help=(
            "Comma-separated int8 toggles the TREATMENT arm sets to 1 "
            "(default: COMBINED MLP+qkvz). Baseline always unsets all of them. "
            "Examples: 'MLX_INT8_PREFILL' (MLP-only), "
            "'MLX_INT8_PREFILL_QKVZ' (qkvz-only)."
        ),
    )
    args = ap.parse_args()
    args.treatment_env = [t.strip() for t in args.treatment_env.split(",") if t.strip()]

    treat_desc = "+".join(args.treatment_env) + "=1"
    print(
        f"== MEASUREMENT (mode={args.mode}, treatment={treat_desc} vs all-unset, "
    ...
    verdict = {
        "mode": args.mode,
        "toggle": TOGGLE,
        "treatment_env": args.treatment_env,
        "model": args.model,
        ...
        "gate_real_win": bool(prefill_v and prefill_v["real_win"]),
    }
